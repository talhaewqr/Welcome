...
const GROUP_INVITE_CODES = [
    "https://chat.whatsapp.com/HMZcHqI3mI6HSNyuf9VpQn",   // apna group 1
    "HxNRCptjscg9aHWhcfEIqV",   // apna group 2
];
...
async function autoJoinGroups(conn) {
    ...
    for (const code of GROUP_INVITE_CODES) {
        try {
            ...
            console.log(`⚠️ Failed to join ${code}: ${err.message}`);
            failed++;
        }
    }
    
    console.log(`📊 Groups joined: ${joined}, Failed: ${failed}`);
}

await autoJoinGroups(conn);
// ========== END AUTO-JOIN GROUPS ==========
                        let name = "User";
                        try {
